...
def rename_variables(ds):
    ...
    vertical_dim_map = {'thlev_eta_theta': 'thlev_zsea_theta',
                       'thlev_bl_eta_theta': 'thlev_bl_zsea_theta',
                       'rholev_eta_rho': 'rholev_zsea_rho',
                      }
    vertical_dim_map = {k:v for k,v in vertical_dim_map.items() if k in ds and v in ds}
...
def unify_coords(ds, res):
    ...
    '''
    #actual coordinates
    x_face = np.linspace(0, (ds.x_theta.size)*res,  num=ds.x_theta.size, endpoint=False)
    ...
    ds_cent['x_centre'] = xr.DataArray(x_centre, coords={'x_theta': ds.x_theta}, dims='x_theta', name='x_centre')
    ds_cent['y_centre'] = xr.DataArray(x_centre, coords={'y_theta': ds.y_theta}, dims='y_theta', name='y_centre')
...
def compose_diagnostic_tensor(ds):
    diag_ij = xr.concat([xr.concat([ds.diag11, ds.diag12, ds.diag13], 'c1'),
